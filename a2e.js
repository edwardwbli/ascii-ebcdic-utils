...
var a2e = {
      ...
      asciiStrToEbcdicChr: function (input) {
        var hexArr = this.asciiStrToEbcdicStr(input);
        var ebcdicChr = []
        hexArr.forEach(function(hex){
          
          ebcdicChr.push(String.fromCharCode(parseInt(hex,16)));
        })
      
        /*
        ebcdicChr.forEach(function(input) {
          console.log(input.charCodeAt(0).toString(16))
        })*/
        return ebcdicChr.join('');
      },
      ebcdicStrToAsciiChr: function (input) {
        input = input.split('');

        var asciiChr = [];

        input.forEach(function (character) {
          asciiChr.push(hexMap[character.charCodeAt(0).toString(16).toUpperCase()].ebcdic)

        });
        return asciiChr.join('');
      }
    };